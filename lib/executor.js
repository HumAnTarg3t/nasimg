...
const { hashFile: defaultHashFile } = require("./hash");
...
function createExecutor({
  ...
  async function statDestIn(dir, name) {
    ...
    if (claimed.has(claimKey(target))) {
      return { size: claimed.get(claimKey(target)), claimed: true };
    }
  ...
  async function processFile(file) {
    ...
    try {
      ...
      // Hashed at most once per file, and only if a name+size collision is
      // actually hit. A read error propagates rather than being read as "not a
      // duplicate": failing the file is safe, it is retried next run.
      let sourceHash = null;
      const sameContent = async (name) => {
        if (sourceHash === null) sourceHash = await hashFile(file.sourcePath);
        return sourceHash === (await hashFile(path.join(dateDir, name)));
      };
